...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        {
            Position.x += velocity * cos(glm::radians(Yaw));
            Position.z += velocity * sin(glm::radians(Yaw));
        }
        ...
        if (direction == UP && Position.y == 0.0f)
        {
            yVelocity += jumpSpeed;
            Position.y += 0.05f;
        }
...
public:
    ...
    void updateCameraVectors(float deltaTime)
    {
        ...
        if (Position.y < 0.0f && onSurface())
        {
            Position.y = 0.0f;
            yVelocity = 0.0f;
        }

        // If fallen off, respawn
        if (Position.y <= -50.0f)
        {
    ...
    bool onSurface()
    {
        return Position.x >= -5.0f && Position.x <= 5.0f
